...
class Solution {
    public List<List<Integer>> kSmallestPairs(int[] nums1, int[] nums2, int k) {
        ...
        for (int num1 : nums1) {
            for (int num2 : nums2) {
                pairAndSumList.add(List.of(num1, num2), num1 + num2)
            }
        }
        Comparator<PairAndSum> comparator = Comparator.comparing(PairAndSum::gum);
        ...
        for (int i = 0; i < k; i++) {
            kSmallestPairs.add(sortedList.get(i).getNumPair());
        }
